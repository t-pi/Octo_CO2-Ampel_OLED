 ...
 *  2022-01-12: VMini for SCD30/BME680 Featherwing on ESP32 Huzzah + QWIIC OLED
 */
#include <EEPROM.h>
#include <Wire.h>
#include <WiFi.h>
...
//#define NEO_PIN 13  // Octopus CO2-Ampel https://www.tindie.com/products/FabLab/diy-octopus-with-scd30-and-nodemcu-v2/
//Adafruit_NeoPixel pixels = Adafruit_NeoPixel(2,NEO_PIN,NEO_GRBW + NEO_KHZ800);
#define NEO_PIN 18  // CO2/IAQ Wing https://www.tindie.com/products/fablab/co2iaq-wing-neopixelbme680-for-scd30-or-mh-z19/
Adafruit_NeoPixel pixels = Adafruit_NeoPixel(1,NEO_PIN,NEO_GRBW + NEO_KHZ800);
#define NEO_IAQ 0
#define NEO_CO2 1 // not needed for Mini CO2-Ampel
int brightness = 100;

//  Adafruit_SSD1306 display = Adafruit_SSD1306(128, 32, &Wire);  // Featherwing OLED
Adafruit_SSD1306 display = Adafruit_SSD1306(128, 64, &Wire);  // Separate OLED, only using 32 lines
...
//  if (Wire.status() != I2C_OK) Serial.println("Something wrong with I2C");
  ...
  airSensorSCD30.setAutoSelfCalibration(true); // Sensirion auto calibration
  airSensorSCD30.setMeasurementInterval(2);     // CO2-Messung alle 2 s

  // --- Init NeoPixels
  Serial.println("NeoPixels init...");
  pixels.begin();
  delay(1);
...
//  pixels.setPixelColor(NEO_CO2,0,0,0,0);
  ...
  display.begin(SSD1306_SWITCHCAPVCC, 0x3C); // Address 0x3C default, Featherwing & separate OLED
...
// Not usable in Mini CO2-Ampel - no button...
...
// Not usable for Mini CO2-Ampel - no button!
...
//  Get maximum of both values for LED color
  int myIAQ = int(float(IAQ)/250.0*1200);
  long color = get_CO2color(max(CO2, myIAQ)); // 0xGGRRBB
